...
describe("buildCacheKey", () => {
  it("keeps version, model, level, persona, and normalized prompt", () => {
    const key = buildCacheKey(8, { ...base, persona: "dev" });
    expect(key).toBe("v8|gpt-5|2|dev||fix the bug");
  });

  it("includes standing context (pre-existing gap fixed)", () => {
    const a = buildCacheKey(8, base);
    const b = buildCacheKey(8, { ...base, context: "B2B SaaS audience" });
    expect(a).not.toBe(b);
    expect(b).toContain("|B2B SaaS audience|");
  });

  it("adds |terminal and |type: markers as before", () => {
    expect(buildCacheKey(8, { ...base, terminalContext: true })).toContain("|terminal");
    expect(buildCacheKey(8, { ...base, promptType: "question" })).toContain("|type:question");
    expect(buildCacheKey(8, { ...base, promptType: "auto" })).not.toContain("|type:");
  });
  ...
  it("same prompt with vs without captureContext gets different keys", () => {
    ...
    const a = buildCacheKey(8, base);
    const b = buildCacheKey(8, { ...base, captureContext: ctx });
    expect(a).not.toBe(b);
  ...
  it("suggestedModel does not affect the key", () => {
    ...
    const a = buildCacheKey(8, { ...base, captureContext: ctx });
    const b = buildCacheKey(8, { ...base, captureContext: { ...ctx, suggestedModel: "gpt-5" } });
    expect(a).toBe(b);
  });

  it("empty captureContext hashes like no context", () => {
    expect(buildCacheKey(8, { ...base, captureContext: {} })).toBe(buildCacheKey(8, base));
  });

  it("context fields that reach the meta-prompt all change the key", () => {
    const mk = (ctx: CaptureContext) => buildCacheKey(8, { ...base, captureContext: ctx });
    const keys = [
